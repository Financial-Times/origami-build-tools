...
function getMainSassPath() {
	...
	var fileExists = fs.existsSync(sassMainPath);
...
function getBowerrc() {
	//can't use requireIfExists as doesn't end in .js or .json
	var pathToBowerrc = path.join(process.cwd(), '/.bowerrc');

	if (fs.existsSync(pathToBowerrc)) {
		try {
			return JSON.parse(fs.readFileSync(pathToBowerrc, 'utf8'));
		} catch (e) {
			throw '.bowerrc is invalid json';
		}
	} else {
		return undefined;
	}
