...
module.exports._runBrowserTest = function(config) {
	return new Promise(function(resolve, reject) {
		...
		fs.readFile(nightwatchConfigPath, 'utf8', function(err, configContent) {
			...
			fs.writeFile(nightwatchConfigPath.replace('json', 'tmp.json'), JSON.stringify(configContent), 'utf8', function(error) {
				if (error) {
					reject(error);
				}
				...
				resolve(commandLine.run(nightwatchCommand, [
						'--group', testsPath,
						'--config', nightwatchConfigPath.replace('json', 'tmp.json'),
						'--env', environments
				]));
			});
		});
