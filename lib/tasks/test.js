...
module.exports = function(gulp) {
	...
	module.exports.browserTest(gulp);
};
...
function runNightwatch(gulp, config) {
	return new Promise(function(resolve, reject) {
		...
		var nightwatchConfig = fs.readFile(nightwatchConfigPath, function(err, configContent) {
			if (err) {
				reject(err);
			}

			configContent = JSON.parse(configContent);
			configContent.default.launch_url = config.testUrl;

			fs.writeFile(nightwatchConfigPath, function(err) {
				if (err) {
					reject(err);
				}

				return commandLine.run(nightwatchCommand, ['--group', 'test/browser/tests',
...
module.exports.browserTest = function(gulp, config) {
	...
	.then(function() {
		...
		return runNightwatch(gulp, config);
	})
	// .then(function(app) {
	// 	console.log(1, app);
