...
module.exports.pa11yTest = function(gulp, config) {
	...
	// ignoring by default warnings, notices and the void href on links error
	const defaultIgn = ['WCAG2AA.Principle2.Guideline2_4.2_4_1.G1,G123,G124.NoSuchID', 'warning', 'notice'];
	let src = (config.pa11yPath !== undefined) ? path.join(process.cwd(), config.pa11yPath) : defaultSrc;
	let ign = (config.pa11yIgnore !== undefined) ? config.pa11yIgnore.split(';') : defaultIgn;

	return new Promise(function(resolve, reject) {
		try {
			fs.readFile(src, function(err) {
				if (err === null) {
					const test = pa11y({
						log: {
					...
					src = 'file://' + src;
					test.run(src, function(error, result) {
						if (error) {
							reject(error);
						} else {
							pa11yCliReporter.results(result, src);
							resolve(result);
						}
					});
				} else {
					log.primary(`Could not open ${src}`);
					resolve(err);
				};
			});
		} catch(err) {
			log.secondaryError('error', err);
		};
	});
};
