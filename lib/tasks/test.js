...
module.exports.pa11yTest = function(gulp, config) {
	...
	let ignore = (config.pa11yIgnore !== undefined) ? config.pa11yIgnore.split(';') : defaultIgnore;
	...
	return new Promise(function(resolve, reject) {
		fs.readFile(src, function(err) {
			if (err === null) {
				...
				src = (src.indexOf('file://') === -1 && src.indexOf('http://') === -1 && src.indexOf('https://') === -1 ) ? 'file://' + src : src;
