...
function lintspacesFailReporter() {
	var logLintspacesErrors = function(error, path) {
		console.error(
			'[%s] %s in (%s:%d)\n',
...
module.exports.eslint = function(gulp, config) {
	config = config || {};
	var configPath = config.eslintPath || path.join(__dirname, '/../../config/eslint.json');
	excludeFiles = excludeFiles.concat(config.excludeFiles || []);
	return gulp.src(['**/*.js'].concat(excludeFiles))
		.pipe(eslint({
			configFile: configPath
		}))
		.pipe(eslint.format('compact', process.stdout))
