...
module.exports = function buildSass(config) {
	...
	return src.then(code => {
		if (code) {
			...
			config.flags.production = config.flags.production || 'development';
			...
			// 			.on('error', reject);
			// 		})
			const useSourceMaps = config.sourcemaps || config.flags.production === 'development';
			...
			return readFile(code, 'utf-8')
				.then(sassFile => {
					...
					if (config.flags && config.flags.production === 'production') {
