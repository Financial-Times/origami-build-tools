		...
		if (code) {
			...
			return readFile(code, 'utf-8')
				.then(sassFile => {
					...
					const includePaths = (config.sassIncludePaths || []).concat('bower_components', 'node_modules', 'node_modules/@financial-times').map(p => path.join(cwd, p));

					// Ensure that the specified current working directory is included as the first path for node-sass to look into
