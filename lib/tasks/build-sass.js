		...
		if (sassFile) {
			...
			return Promise.resolve(sassData)
				.then(sassData => {
					...
					// post css does not parse the charset correct, unless it is
					// also base64. Remove this when postcss release a fix.
					...
					if (destFolder !== 'disabled') {
						return outputFile(path.join(destFolder, dest), postcssResult);
					} else {
