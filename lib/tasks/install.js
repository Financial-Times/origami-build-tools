...
function listBowerVersions () {
	var componentsDir = (files.getBowerrc() || {}).directory || 'bower_components';
	var path = require('path');
	return require('fs').readdirSync(componentsDir).map(function (component) {
		var data = require(path.join(process.cwd(), componentsDir, component, '.bower.json'));
		return data.name + ': ' + data.version;
	}).join('\n');

}

function cliSuccess(info) {
	log.primary('Install successful.');
	info && log.secondary(info);
}
...
module.exports.description = 'Install system and local dependencies';
