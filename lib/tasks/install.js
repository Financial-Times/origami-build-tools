    ...
    getInstalledBowerVersion()
    	.then(function(version) {
	        if (version === -1 || semver.lt(version, versions.bower)) {
	            commandLine.run('npm', ['install', 'bower', '--quiet'], { stderr: 'pipe', stdout: 'pipe' })
					.then(function() {
						outputInstalling();
						cliSuccess();
					}, function(output) {
...
module.exports.installJshint = function() {
	...
	files.getNodeModulesDirectoryInUse()
		.then(function(nodeModulesPath) {
			var jsHint = path.join(nodeModulesPath, '/.bin/jshint');

			getCommandVersion(jsHint, '--version')
				.then(function(version) {
					if (version === -1 || semver.lt(version, versions.JSHint)) {
						commandLine.run('npm', ['install', 'jshint'])
							.then(function() {
								outputInstalling();
								cliSuccess();
							}, function(output) {
