...
module.exports = function(gulp, config) {
	...
	module.exports.installBower()
		.then(function() {
			module.exports.runBowerInstall(config && config.verbose ? { verbose: true } : undefined);
		});
...
module.exports.installSass = function() {
	...
	getInstalledSassGemVersion()
		.then(function(version) {
			if (version === -1 || semver.satisfies(version, versions.sass)) {
				commandLine.run('gem', ['install', 'sass', '-v', bestGems.sass])
					.then(function() {
						outputInstalling();
						cliSuccess();
					}, function(output) {
...
module.exports.installJshint = function() {
	...
	getInstalledJshintVersion()
		.then(function(version) {
			if (version === -1 || semver.satisfies(version, versions.JSHint)) {
...
module.exports.runBowerInstall = function(config) {
	function outputInstalling(configContainsOrigamiRegistry) {
		...
		if (!configContainsOrigamiRegistry) {
			...
			log.secondaryError('Running the following will do this for you');
			log.secondary('echo \'{"registry":{"search":["http://registry.origami.ft.com","https://bower.herokuapp.com"]}}\' > ~/.bowerrc');
