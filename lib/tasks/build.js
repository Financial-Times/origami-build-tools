	...
	if (src) {
		...
		var transforms = [
			...
			}),
			{
				...
				options: { bowerOptions: { cwd: cwd, relative: true }}
			},
		...
		bundle = transforms.reduce(function(bundler, transform) {
			if (transform.options && transform.transform) {
				return bundler.transform(transform.transform, transform.options);
			}
