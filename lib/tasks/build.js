...
module.exports.js = function(gulp, config) {
	...
	if (src) {
		...
		const loaders = [
			...
			'babel' + (babelRuntime ? '?optional[]=runtime' : ''),
		...
		if (config.env === 'production') {
			plugins.push(new webpackStream.webpack.optimize.UglifyJsPlugin({
				sourceMap: useSourceMaps
			}));
			plugins.push(new webpackStream.webpack.DefinePlugin({ 'process.env': { 'NODE_ENV': '"production"' } }));
		}
		const webpackConfig = {
			quiet: true,
			resolve: {
