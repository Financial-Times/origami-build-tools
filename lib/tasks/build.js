...
module.exports.js = function(gulp, config){
	...
	if (src) {
		...
		if (config.hash) {
			if (!config.manifest) {
				log.primaryError('JS file creation failed - hashing requested but manifest name not present');
			} else {
				es.merge(
					...
					bundle.bundle(config)
		                ...
		                .pipe(gulpif(config.hash, hash.manifest(config.manifest)))
				)
				.pipe(gulpExtend(config.manifest, false, 4))
		...
		log.primaryError('JS file creation failed - no source available');
	}
...
module.exports.sass = function(gulp, config) {
	...
	if (src) {
		...
		if (config.hash) {
			if (!config.manifest) {
				log.primaryError('Sass file creation failed - hashing requested but manifest name not present');
				return;
			}
			...
			return es.merge(
				...
				gulp.src(src)
					...
					.pipe(gulp.dest(destFolder))
					.pipe(hash.manifest(config.manifest))
		...
		log.primaryError('Sass file creation failed - no source available');
	}
};
