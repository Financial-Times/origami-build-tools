...
module.exports.js = function(gulp, config) {
	...
	if (src) {
		...
		// Currently using forked transform-loader with Browserify textrequireify
		// until webpack bug mentioned in textrequireify-loader.js is fixed
		var textrequireifyPath = path.join(__dirname, '../../node_modules/textrequireify/index.js');
		var loaders = ['transform?' + textrequireifyPath + '=>{rootDirectory: "' + cwd + '"}', 'babel-runtime-path-loader', 'babel?optional[]=runtime'].concat(config.loaders || []);
		...
		var stream = gulp.src(src)
			.pipe(webpack(webpackConfig, null, function(err, stats) {
				if (stats.compilation.errors) {
					// Logs errors that don't break builds, need to figure out how to break them
					// but the erros are thrown after the build is made anyway
