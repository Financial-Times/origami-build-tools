...
module.exports.sass = function(gulp, config) {
	...
	if (src) {
		...
		if (useSourceMaps === true) {
			log.secondary('Separate file source maps not yet supported for sass. Outputting inline sourcemap.');
		}
		...
		const combinedStream = combine(
			...
			gulpif(useSourceMaps, function() {
				if (useSourceMaps === 'inline') {
					...
					return sourcemaps.write('./', { sourceMappingURL: file => `${dest}.map` });
				}
			}),
			rename(path => {
				if (path.extname !== '.map') {
