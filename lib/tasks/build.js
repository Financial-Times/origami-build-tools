...
module.exports.js = function(gulp, config){
	...
	if (src) {
		...
		if (config.hashManifest) {
			es.merge(
				gulp.src(config.hashManifest),
				bundle.bundle(config)
					.pipe(source(dest))
					.pipe(gulpif(config.env === 'production', streamify(uglify())))
					.pipe(streamify(hash(hashOptions)))
					.pipe(gulp.dest(destFolder))
	                .pipe(hash.manifest(config.hashManifest))
			)
			.pipe(gulpExtend(config.hashManifest, false, 4))
			.pipe(gulp.dest(destFolder));
		} else {
			bundle.bundle(config)
				.pipe(source(dest))
