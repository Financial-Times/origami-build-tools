...
module.exports = (cfg = {}) => {
	return Promise.all([files.getBowerJson(), files.getPackageJson(), files.getMainJsPath()])
		.then(([bower, npm = {}, mainJs]) => {
			...
			// Bail early and don't rewrite if there's an `origami.noRewrite` attribute
			if (pkg.origami && pkg.origami['noRewrite']) {
				return;
			}
			...
			const newDeps = Object.keys(dependencies).reduce((acc, key) => {
				acc[`@Financial-Times/${key}`] = `github:Financial-Times/${key}#semver:${dependencies[key]}`;
				return acc;
			}, {});
