...
function buildHtml(gulp, demoConfig, staticSource) {
	return new Promise(function(resolve, reject) {
		...
		var browserFeatures = ['default'];
		if (origamiJson.browserFeatures && origamiJson.browserFeatures.required) {
			browserFeatures = browserFeatures.concat(origamiJson.browserFeatures.required);
		}
		if (origamiJson.browserFeatures && origamiJson.browserFeatures.optional) {
			browserFeatures = browserFeatures.concat(origamiJson.browserFeatures.optional);
		}
...
module.exports = function(gulp, config) {
	...
	if (configPath) {
		...
		if (!Array.isArray(demosConfig.demos)) {
			...
			console.log(JSON.stringify(demosConfig.demos, null, 2));
		...
		if (!hasUniqueNames(demosConfig.demos)) {
			log.primaryError("Demos with the same name were found. Give them unique names and try again.");
			return;
		}
