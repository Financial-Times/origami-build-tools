...
function buildSass(buildConfig) {
	...
	if (builtFiles.css.indexOf(src) === -1) {
		...
		return streamFromPromise(buildsass(sassConfig));
	}
}
...
function buildJs(gulp, buildConfig) {
	...
	if (builtFiles.js.indexOf(src) === -1) {
		...
		return streamFromPromise(buildJS(jsConfig));
	}
}
...
function buildHtml(gulp, buildConfig) {
	...
	try {
		...
		throw new Error(e + ' in ' + origamiJsonPath);
	}
...
module.exports = function(gulp, config) {
	...
	if (configPath) {
		...
		const demosConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
		const demos = [];
		builtFiles.css = [];
		builtFiles.js = [];
		...
		demos.forEach(function(demo) {
			...
			if (config.dist) {
				...
				if (demo.sass && (typeof config.watching === 'undefined' || config.watching === 'sass')) {
					const sassStream = buildSass(buildConfig);
					if (sassStream) {
						sassStream
							.on('error', emitDemoStreamErrorEvent);
