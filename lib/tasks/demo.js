...
function buildHtml(gulp, buildConfig) {
	...
	const dest = 'demos/' + (buildConfig.staticSource === 'local' ? 'local/' : '');
	...
	data.oDemoTitle = files.getModuleName() + ': ' + buildConfig.demo.name + ' demo';
	data.oDemoStyle = getStylesheetTags(buildConfig.demo.sass, buildConfig.demo.dependencies, buildConfig.staticSource);
	data.oDemoScript = getScriptTags(buildConfig.demo.js, buildConfig.demo.dependencies, buildConfig.staticSource);
...
function getStylesheetTags(sassPath, dependencies, staticSource) {
	...
	if (staticSource === 'local') {
		...
		stylesheets += '<link rel="stylesheet" href="/bundles/css?modules=';
		if (sassPath) {
			stylesheets +=	files.getModuleName() +
...
function getScriptTags(scriptPath, dependencies, staticSource) {
	...
	if (staticSource === 'local') {
		...
		scripts += '<script src="/bundles/js?modules=';
		if (scriptPath) {
			scripts += files.getModuleName() +
...
module.exports = function(gulp, config) {
	...
	if (configPath) {
		...
		if (configPath === 'origami.json') {
			...
			fs.writeFileSync(path.join(cwd, '/' + configPath), origamiJson, { encoding: 'utf8' });
		...
		demos.map(function(demo) {
			...
			if (config.local) {
				buildConfig.staticSource = 'local';
