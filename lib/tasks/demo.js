...
function buildHtml(gulp, demoConfig, staticSource) {
    ...
    if (fs.existsSync(src) && builtFiles.html.indexOf(dest) === -1) {
        ...
        return gulp.src(__dirname + '/../../templates/page.mustache')
            .pipe(mustache(data))
            // We run mustache twice so that partials that affect the template are also compiled. 
            // Another option would be to replace {{{oDemoTpl}}} with the partial, and then 
