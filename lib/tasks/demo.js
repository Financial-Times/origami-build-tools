	...
	return new Promise(function(resolve, reject) {
		...
		if (builtFiles.js.indexOf(src) === -1) {
			if (!fs.existsSync(src)) {
				throw new Error("Can't find javascript file at " + src);
			}
...
function buildHtml(gulp, demoConfig, staticSource) {
	return new Promise(function(resolve, reject) {
		...
		if (fs.existsSync(src)) {
			...
			if (demoConfig.sass) {
				data.oDemoStyle = getStylesheetHref(demoConfig.sass, demoConfig.dependencies, staticSource);
			}
			if (demoConfig.js) {
				data.oDemoScript = getScriptSrc(demoConfig.js, demoConfig.dependencies, staticSource);
			}
...
function getStylesheetHref(sassPath, dependencies, staticSource) {
	var stylesheets = '';
	if (staticSource === 'local') {
		if (dependencies) {
...
function getScriptSrc(scriptPath, dependencies, staticSource) {
	var scripts = '';
	if (staticSource === 'local') {
		if (dependencies) {
