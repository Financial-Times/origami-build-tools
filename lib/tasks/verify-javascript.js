...
async function lint (config) {
	...
	if (hasJS) {
		...
		const eslint = new ESLint({
			overrideConfigFile: path.join(config.cwd, '/.eslintrc.js'),
			useEslintrc: false
		});
...
module.exports = function (cfg) {
	...
	return {
		...
		skip: async () => {
			...
			return fileExists(path.join(config.cwd, '.eslintrc.js')).then(exists => {
				if (!exists) {
					return `No .eslintrc.js file found. To enable JavaScript linting create a file at ${path.join(config.cwd, '/.eslintrc.js')}. Something like ${'`module.exports = { "extends": "origami-component" };`'}`;
				}
			});
		}
	};
