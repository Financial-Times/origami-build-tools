	...
	return constructPolyfillUrl()
		.then(polyfillUrl => {
			return new Promise((resolve, reject) => {
				...
				getCustomKarmaConfig()
					.then(customKarmaConfig => {
						const cfg = require('karma').config;
						Object.assign(customKarmaConfig, { basePath: config.cwd });
						resolve(cfg.parseConfig(null, customKarmaConfig));
					}).catch(() => {
						reject(new Error(`Unable to resolve custom Karma Config.`));
					});
			}).then((karmaConfig) => {
				...
				const server = new Server(karmaConfig, exitCode => {
					if (exitCode !== 0) {
						throw new Error(`Failed Karma tests: ${'\n\n' + errors.join('\n\n')}`);
					}
				});
				...
				server.on('browser_error', function (browser, error) {
					...
					throw new Error(`Error connecting to ${browser.name}: ${error.toString()}`);
				});
