...
async function origamiJson(config) {
	...
	const remarkFile = '.remarkrc.js';
	const remarkPath = path.join(config.cwd, remarkFile);
	const hasRemark = await fs.exists(remarkPath);
	if (!hasRemark) {
		throw new Error(`Could not lint the README. The component is missing lint configuration "${remarkFile}".`);
	}
