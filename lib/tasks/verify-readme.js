...
module.exports = cfg => {
	...
	return {
		...
		skip: () => {
			return fileExists(path.join(config.cwd, '.remarkrc.js')).then(exists => {
				if (!exists) {
					return `No .remarkrc.js file found. To enable readme linting create a file at ${path.join(config.cwd, '/.remarkrc.js')}. Something like ${'`module.exports.plugins = [require("remark-preset-lint-origami-component")]`'}`;
				}
			});
		}
	};
