...
module.exports = function (cfg) {
	...
	return files.getModuleBrands().then((brands) => {
		const sassTests = [];
		const sassImplementations = ['node-sass', 'dart-sass'];

		for (const sassImplementation of sassImplementations) {
			sassTests.push({
				title: `Testing ${sassImplementation} compilation with silent mode on`,
			...
			if (brands.length === 0) {
				sassTests.push({
					title: `Testing ${sassImplementation} compilation with silent mode off`,
					task: () => compilationTest(config.cwd, {
						sassImplementation,
						silent: false,
						ignoreBower: config.ignoreBower
			...
			if (brands.length > 0) {
				sassTests.push(...brands.map(brand => {
					return {
						title: `Testing ${sassImplementation} compilation for the ${brand} brand with silent mode off`,
						task: () => compilationTest(config.cwd, {
							brand,
							sassImplementation,
							silent: false,
