...
function buildSass(demoConfig, callback) {
    ...
    if (fs.existsSync(src) && builtFiles.css.indexOf(dest) === -1) {
        ...
        if (bowerConfig.dependencies['o-assets']) {
            var moduleName = bowerConfig.name;
            var sourceSassFile = fs.readFileSync(src, 'utf8');
            // Create a modified temporary sass file with the oAssets mixin that does this appended to it
            src = src.replace('.scss', '.temp.scss');
        ...
        sass.compile(src, dest, { 
                sourcemap: true 
            },
            function () {
                builtFiles.css.push(dest);
                // If it's using a temporary sass file, delete it
                if (src.indexOf('.temp.scss') !== -1) {
                    fs.unlinkSync(src);
                }
                callback();
            });
...
function buildDemos(configPath, buildLocal, updateOrigami, callback) {
    ...
    if (getConfig(configPath)) {
        log.primary("Build " + (buildLocal ? " local and" : "") + " buildservice demos using config file at " + configPath);
