...
function runLocalDemosServer() {
    // Run server in the root of the component so all assets load properly
    console.log(path.resolve(__dirname, '../../node_modules/http-server/bin/http-server'));
    var proc = require('child_process').spawn(path.resolve(__dirname, '../../node_modules/http-server/bin/http-server'), [process.cwd()]);
