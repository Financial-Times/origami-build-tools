...
function getCommandVersion(command, versionFlag, callback) {
    "use strict";
    commandLine.run(command, [versionFlag], { stderr: process.stdout }, function(error, output) {
        console.dir(output.stdout.toString());
        if (!error) {
            ...
            if (files.bowerJsonExists()) {
                ...
                var bowerCommand = getBowerCommand(function(err, command) {
					...
					if (!configContainsOrigamiRegistry) {
						...
						process.nextTick(function() { callback('EBUILDFAILED'); } );
						return;
					}
