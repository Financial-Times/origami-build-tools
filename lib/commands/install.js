...
function getInstalledBowerVersion(callback) {
    "use strict";
    commandLine.run(process.cwd() + '/node_modules/bower/bin/bower', ['--version'], function(err, result) {
        if (!err) {
    ...
    async.series([
        ...
        function(callback) {
            if (files.bowerJsonExists()) {
                ...
                commandLine.run(process.cwd() + '/node_modules/bower/bin/bower', ['install', '--config.registry.search=http://registry.origami.ft.com', '--config.registry.search=https://bower.herokuapp.com'], callback);
            } else {
