...
function getGemVersion(gem, callback) {
    "use strict";
    commandLine.run(gem, ['--version'], function(error, output) {
        if (error) {
            callback(-1);
        } else {
...
function getInstalledBowerVersion(callback) {
    "use strict";
    commandLine.run(path.join(process.cwd(), '/node_modules/bower/bin/bower'), ['--version'], function(error, output) {
        if (!error) {
            callback(output.stdout.trim() || -1);
        } else {
            callback(-1);
        }
    });
