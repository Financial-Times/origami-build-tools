        ...
        function(callback) {
            if (files.bowerJsonExists()) {
                ...
                var bowerCommand = getBowerCommand(function(err, command) {
					...
					var registrySearchConfig = new BowerConfig(process.cwd()).load()._config.registry.search;

					var configContainsOrigamiRegistry = Array.isArray(registrySearchConfig) &&
														((registrySearchConfig.indexOf("http://registry.origami.ft.com") > 0) ||
														(registrySearchConfig.indexOf("https://registry.origami.ft.com") > 0) ||
														(registrySearchConfig.indexOf("registry.origami.ft.com") > 0));
					...
					if (!configContainsOrigamiRegistry) {
						...
						process.nextTick(function() { callback("Check Origami registry configuration in bowerrc"); } );
						return;
					}
